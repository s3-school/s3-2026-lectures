...
Congratulations! We have successfully packaged `pkoffe` for the conda ecosystem! `pkoffee-0.1.0-pyh4616a5c_0.conda` is `pkoffee` package, we can now distribute it to our users. There are a few options to do this, which one to chose depends on the scope and visibility of our project:
